...
import com.test.BTClient.MyImageTopView;
...
public class BTClient extends Activity {
	...
	private int[] imgIds = new int[] { R.drawable.a, R.drawable.b,
			R.drawable.c};//需要显示的一组图片的ID
    ...
    public void initBottom() {// 初始化底部的小圆圈，并为圆圈添加单击事件处理	
		for (int i = 0; i < imgViews.length; i++) {
			imgViews[i] = new ImageView(this);//创建ImageView显示圆圈
			if (i == 0) {//默认选中第一个
			...
			imgViews[i].setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					resetImg();//使所有的圆圈都不选中
					((ImageView) v).setImageResource(R.drawable.choosed);//将当前圆圈设为选中状态
